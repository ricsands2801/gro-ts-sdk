...
```sh
npm install git+ssh://git@github.com:stainless-sdks/gro-typescript.git
```
...
const api: string = await client.api.retrieve();
```
...
<!-- prettier-ignore -->
```ts
const api = await client.api.retrieve().catch(async (err) => {
  if (err instanceof Gro.APIError) {
...
const response = await client.api.retrieve().asResponse();
console.log(response.headers.get('X-My-Header'));
console.log(response.statusText); // access the underlying Response object

const { data: api, response: raw } = await client.api.retrieve().withResponse();
...
1. Via the `GRO_LOG` environment variable
...
We are keen for your feedback; please open an [issue](https://www.github.com/stainless-sdks/gro-typescript/issues) with questions, bugs, or suggestions.
