...
export interface ClientOptions {
  /**
   * Defaults to process.env['GRO_API_KEY'].
   */
  apiKey?: string | null | undefined;

  /**
   * Defaults to process.env['GRO_BEARER_TOKEN'].
   */
   ...
   *
   * Defaults to process.env['GRO_BASE_URL'].
   */
   ...
   * Set the log level.
   *
   * Defaults to process.env['GRO_LOG'] or 'warn' if it isn't set.
   */
...
/**
 * API Client for interfacing with the Gro API. 
 */
export class Gro {
  apiKey: string | null;
   ...
   * API Client for interfacing with the Gro API.
   *
   * @param {string | null | undefined} [opts.apiKey=process.env['GRO_API_KEY'] ?? null]
   * @param {string | null | undefined} [opts.bearerToken=process.env['GRO_BEARER_TOKEN'] ?? null]
   * @param {string} [opts.baseURL=process.env['GRO_BASE_URL'] ?? https://api.example.com] - Override the default base URL for the API.
   ...
   */
  constructor({
    baseURL = readEnv('GRO_BASE_URL'),
    apiKey = readEnv('GRO_API_KEY') ?? null,
    bearerToken = readEnv('GRO_BEARER_TOKEN') ?? null,
    ...opts
  }: ClientOptions = {}) {
    ...
    this.timeout = options.timeout ?? Gro.DEFAULT_TIMEOUT /* 1 minute */;
    this.logger = options.logger ?? console;
    const defaultLogLevel = 'warn';
    // Set default logLevel early so that we can log a warning in parseLogLevel.
    this.logLevel = defaultLogLevel;
    this.logLevel = parseLogLevel(options.logLevel, 'ClientOptions.logLevel', this) ?? parseLogLevel(readEnv('GRO_LOG'), 'process.env[\'GRO_LOG\']', this) ?? defaultLogLevel;
  ...
  static GroError = Errors.GroError;
