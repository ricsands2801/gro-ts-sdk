...
import { APIPromise } from 'gro-typescript/core/api-promise';

import util from 'node:util';
import Gro from 'gro-typescript';
import { APIUserAbortError } from 'gro-typescript';
...
describe('logging', () => {
  ...
  const forceAPIResponseForClient = async (client: Gro) => {
  ...
  test('default logLevel is warn', async () => {
    const client = new Gro({ apiKey: 'My API Key' });
  ...
  test('debug logs happen with debug env var', async () => {
    ...
    process.env['GRO_LOG'] = 'debug';
    const client = new Gro({ logger: logger, apiKey: 'My API Key' });
  ...
  test('warn when env var level is invalid', async () => {
    ...
    process.env['GRO_LOG'] = 'not a log level';
    const client = new Gro({ logger: logger, apiKey: 'My API Key' });
    expect(client.logLevel).toBe('warn');
    expect(warnMock).toHaveBeenCalledWith('process.env[\'GRO_LOG\'] was set to "not a log level", expected one of ["off","error","warn","info","debug"]');
  });
  ...
  describe('baseUrl', () => {
    test('trailing slash', () => {
      const client = new Gro({ baseURL: 'http://localhost:5000/custom/path/', apiKey: 'My API Key' });
    ...
    afterEach(() => {
      process.env['GRO_BASE_URL'] = undefined;
    });

    test('explicit option', () => {
      const client = new Gro({ baseURL: 'https://example.com', apiKey: 'My API Key' });
      expect(client.baseURL).toEqual('https://example.com');
    });

    test('env variable', () => {
      process.env['GRO_BASE_URL'] = 'https://example.com/from_env';
      const client = new Gro({ apiKey: 'My API Key' });
      expect(client.baseURL).toEqual('https://example.com/from_env');
    });

    test('empty env variable', () => {
      process.env['GRO_BASE_URL'] = ''; // empty
      const client = new Gro({ apiKey: 'My API Key' });
      expect(client.baseURL).toEqual('https://api.example.com')
    });

    test('blank env variable', () => {
      process.env['GRO_BASE_URL'] = '  '; // blank
      const client = new Gro({ apiKey: 'My API Key' });
      expect(client.baseURL).toEqual('https://api.example.com')
    });
    ...
    test('in request options overridden by client options', () => {
      const client = new Gro({ apiKey: 'My API Key', baseURL: 'http://localhost:5000/client' });
      expect(client.buildURL('/foo', null, 'http://localhost:5000/option')).toEqual('http://localhost:5000/client/foo');
    });

    test('in request options overridden by env variable', () => {
      process.env['GRO_BASE_URL'] = 'http://localhost:5000/env';
      const client = new Gro({ apiKey: 'My API Key' });
  ...
  test('maxRetries option is correctly set', () => {
    const client = new Gro({ maxRetries: 4, apiKey: 'My API Key' });
    expect(client.maxRetries).toEqual(4);

    // default
    const client2 = new Gro({ apiKey: 'My API Key' });
  ...
  test('with environment variable arguments', () => {
    ...
    process.env['GRO_API_KEY'] = 'My API Key';
    const client = new Gro();
  ...
  test('with overridden environment variable arguments', () => {
    // set options via env var
    process.env['GRO_API_KEY'] = 'another My API Key';
    const client = new Gro({ apiKey: 'My API Key' });
    expect(client.apiKey).toBe('My API Key');
  });
});
...
describe('retries', () => {
  ...
  test('retry on 429 with retry-after', async () => {
    ...
    const client = new Gro({ apiKey: 'My API Key', fetch: testFetch });
